...
public class Task6 {
  ...
  public static Set<String> getPersonDescriptions(Collection<Person> persons,
    ...
    Map<Integer, Area> areaMap = areas.stream().collect(Collectors.toMap(Area::getId, Function.identity()));
    return persons.stream().flatMap(person -> personAreaIds.get(person.getId()).stream()
            .map(areaId -> getDescription(person, areaMap.get(areaId)))).collect(Collectors.toSet());
  }
